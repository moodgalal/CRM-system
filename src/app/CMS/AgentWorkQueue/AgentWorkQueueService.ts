...
export class AgentWorkQueueService {
    ...
    UpdateDataWhereCondition(CM00105) {
        ...
        return this.http.post<KaizenResult>(Url, JSON.stringify(CM00105), { headers: new HttpHeaders().set('Token', localStorage.getItem('token')) });
    }
    DeleteData(AgentID) {
      let Url = `${PublicShared.AppSeverPath}/CMS/Agent/DeleteData?AgentID=${AgentID}`;
